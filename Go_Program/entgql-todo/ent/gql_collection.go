...
func (tq *TodoQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "completed":
			if _, ok := fieldSeen[todo.FieldCompleted]; !ok {
				selectedFields = append(selectedFields, todo.FieldCompleted)
				fieldSeen[todo.FieldCompleted] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
